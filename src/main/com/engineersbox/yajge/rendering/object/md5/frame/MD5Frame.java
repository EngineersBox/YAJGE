...
public class MD5Frame {
    ...
    public String toString() {
        final StringBuilder str = new StringBuilder("frame " + id + " [data: " + System.lineSeparator());
        for (final float data : this.frameData) {
            str.append(data).append(System.lineSeparator());
        }
        str.append(']').append(System.lineSeparator());
        return str.toString();
    }

    public static MD5Frame parse(final String blockId,
                                 final List<String> blockBody) {
        final MD5Frame result = new MD5Frame();
        final String[] tokens = blockId.trim().split("\\s+");
        if (tokens.length < 2) {
            throw new RuntimeException("Invalid frame definition: " + blockId);
        }
