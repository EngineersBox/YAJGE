        ...
        for (int i = 0; i < numLines && !finishHeader; i++) {
            ...
            if (tokens.length > 1) {
                ...
                switch (paramName) {
                    ...
                    case "numFrames" -> header.setNumFrames(Integer.parseInt(paramValue));
                    case "numJoints" -> header.setNumJoints(Integer.parseInt(paramValue));
                    case "frameRate" -> header.setFrameRate(Integer.parseInt(paramValue));
                    case "numAnimatedComponents" -> header.setNumAnimatedComponents(Integer.parseInt(paramValue));
