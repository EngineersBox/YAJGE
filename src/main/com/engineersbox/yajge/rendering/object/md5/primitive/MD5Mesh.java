...
public class MD5Mesh {
    ...
    private static void handleShader(final MD5Mesh mesh,
                                     final String line) {
    ...
    private static void handleVertex(final List<MD5Vertex> meshVertices,
                                     final String line) {
    ...
    private static void handleTriangle(final List<MD5Triangle> meshTriangles,
                                       final String line) {
    ...
    private static void handleWeight(final List<MD5Weight> meshWeights,
                                     final String line) {
    ...
    public static MD5Mesh parse(final List<String> meshBlock) {
        ...
        final List<MD5Vertex> meshVertices = mesh.getVertices();
        final List<MD5Triangle> meshTriangles = mesh.getTriangles();
        final List<MD5Weight> meshWeights = mesh.getWeights();

        for (final String line : meshBlock) {
            if (line.contains("shader")) {
                handleShader(mesh, line);
            } else if (line.contains("vert")) {
                handleVertex(meshVertices, line);
            } else if (line.contains("tri")) {
                handleTriangle(meshTriangles, line);
            } else if (line.contains("weight")) {
                handleWeight(meshWeights, line);
            }
        }
        return mesh;
    }
